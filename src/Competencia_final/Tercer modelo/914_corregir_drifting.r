...
PARAM  <- list()
PARAM$experimento  <- "DR10142"

PARAM$exp_input  <- "CA10061"

#valores posibles  "ninguno" "rank_simple" , "rank_cero_fijo" , "deflacion"
PARAM$metodo  <- "rank_cero_fijo" #Elijo arbitrariamente cero fijo no tengo experimento para probar
...
#Variables propias y queridas desde la competencia 1
#Creo unas variables segun como arranca el feature, no uso la c por que se complica
colnames<-colnames(dataset)
Lista_m <- colnames[colnames %like% "^m"]## var_x vector que tiene las variables que empiezan con m

Lista_t <- colnames[colnames %like% "^t"]
